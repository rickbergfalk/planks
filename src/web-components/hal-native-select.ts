...
/**
 * HalNativeSelectOption - Individual option for native select
 *
 * Creates a native <option> element inside the parent select.
 */
@customElement("hal-native-select-option")
export class HalNativeSelectOption extends LitElement {
...
/**
 * HalNativeSelectOptGroup - Option group for native select
 *
 * Creates a native <optgroup> element inside the parent select.
 */
@customElement("hal-native-select-optgroup")
export class HalNativeSelectOptGroup extends LitElement {
...
/**
 * HalNativeSelect - A native select web component that mirrors shadcn/ui NativeSelect
 *
 ...
 */
@customElement("hal-native-select")
export class HalNativeSelect extends LitElement {
  @property({ type: String }) value = ""
  ...
  private _buildNativeSelect() {
    ...
    // Process child hal-native-select-option and hal-native-select-optgroup elements
  ...
  private _processChildren(
    ...
    for (const child of children) {
      if (child.tagName.toLowerCase() === "hal-native-select-option") {
        const option = child as HalNativeSelectOption
      ...
      } else if (child.tagName.toLowerCase() === "hal-native-select-optgroup") {
        const optgroup = child as HalNativeSelectOptGroup
...
declare global {
  interface HTMLElementTagNameMap {
    "hal-native-select": HalNativeSelect
    "hal-native-select-option": HalNativeSelectOption
    "hal-native-select-optgroup": HalNativeSelectOptGroup
  }
}
