...
/**
 * HalDrawer - Root container that manages drawer state
 *
 ...
 * ```
 */
@customElement("hal-drawer")
export class HalDrawer extends LitElement {
  @property({ type: Boolean, reflect: true }) open = false

  private _trigger: HalDrawerTrigger | null = null
  private _content: HalDrawerContent | null = null
  private _drawerId = `hal-drawer-${++drawerId}`
  ...
  private _updateChildren() {
    this._trigger = this.querySelector(
      "hal-drawer-trigger"
    ) as HalDrawerTrigger | null
    this._content = this.querySelector(
      "hal-drawer-content"
    ) as HalDrawerContent | null
...
/**
 * HalDrawerTrigger - Element that triggers the drawer on click
 */
@customElement("hal-drawer-trigger")
export class HalDrawerTrigger extends LitElement {
  private _drawer: HalDrawer | null = null
...
/**
 * HalDrawerClose - Element that closes the drawer on click
 */
@customElement("hal-drawer-close")
export class HalDrawerClose extends LitElement {
  createRenderRoot() {
    return this
  }
  ...
  private _handleClick = () => {
    ...
    const drawer = this.closest("hal-drawer") as HalDrawer | null
    drawer?._close()
  }
...
/**
 * HalDrawerContent - The drawer content with overlay
 */
@customElement("hal-drawer-content")
export class HalDrawerContent extends LitElement {
  ...
  private _getSlideInAnimationClass() {
    switch (this._getDirection()) {
      case "top":
        return "animate-hal-drawer-slide-in-from-top"
      case "left":
        return "animate-hal-drawer-slide-in-from-left"
      case "right":
        return "animate-hal-drawer-slide-in-from-right"
      case "bottom":
      default:
        return "animate-hal-drawer-slide-in-from-bottom"
    }
  }
  ...
  private _showContent() {
    ...
    const title = this._contentEl.querySelector("hal-drawer-title")
    if (title) {
      const titleEl = title.querySelector("h2") || title
      titleEl.id = this._titleId
    }

    const description = this._contentEl.querySelector("hal-drawer-description")
    if (description) {
...
/**
 * HalDrawerHeader - Header container for title and description
 */
@customElement("hal-drawer-header")
export class HalDrawerHeader extends LitElement {
  @property({ type: String }) class: string = ""
...
/**
 * HalDrawerFooter - Footer container for actions
 */
@customElement("hal-drawer-footer")
export class HalDrawerFooter extends LitElement {
  @property({ type: String }) class: string = ""
...
/**
 * HalDrawerTitle - Drawer title
 */
@customElement("hal-drawer-title")
export class HalDrawerTitle extends LitElement {
  @property({ type: String }) class: string = ""
...
/**
 * HalDrawerDescription - Drawer description
 */
@customElement("hal-drawer-description")
export class HalDrawerDescription extends LitElement {
  @property({ type: String }) class: string = ""
...
declare global {
  interface HTMLElementTagNameMap {
    "hal-drawer": HalDrawer
    "hal-drawer-trigger": HalDrawerTrigger
    "hal-drawer-close": HalDrawerClose
    "hal-drawer-content": HalDrawerContent
    "hal-drawer-header": HalDrawerHeader
    "hal-drawer-footer": HalDrawerFooter
    "hal-drawer-title": HalDrawerTitle
    "hal-drawer-description": HalDrawerDescription
  }
}
