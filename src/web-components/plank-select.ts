...
/**
 * PlankSelect - Root container that manages select state
 *
 ...
 * ```
 */
@customElement("plank-select")
export class PlankSelect extends LitElement {
  @property({ type: Boolean, reflect: true }) open = false
  @property({ type: String, reflect: true }) value = ""
  @property({ type: Boolean, reflect: true }) disabled = false

  private _trigger: PlankSelectTrigger | null = null
  private _content: PlankSelectContent | null = null
  private _value: PlankSelectValue | null = null
  private _contentId = `plank-select-content-${++selectId}`
  ...
  private _updateChildren() {
    this._trigger = this.querySelector(
      "plank-select-trigger"
    ) as PlankSelectTrigger | null
    this._content = this.querySelector(
      "plank-select-content"
    ) as PlankSelectContent | null
    this._value = this.querySelector(
      "plank-select-value"
    ) as PlankSelectValue | null
  ...
  _getTextForValue(value: string): string {
    const items = this.querySelectorAll("plank-select-item")
    for (const item of items) {
      if ((item as PlankSelectItem).value === value) {
...
/**
 * PlankSelectTrigger - Element that triggers the select on click
 */
@customElement("plank-select-trigger")
export class PlankSelectTrigger extends LitElement {
  @property({ type: String, reflect: true }) size: "sm" | "default" = "default"
  @property({ type: String }) class: string = ""

  private _select: PlankSelect | null = null
...
/**
 * PlankSelectValue - Shows the selected value or placeholder
 */
@customElement("plank-select-value")
export class PlankSelectValue extends LitElement {
...
/**
 * PlankSelectContent - The floating select content
 */
@customElement("plank-select-content")
export class PlankSelectContent extends LitElement {
  ...
  private _renderChild(child: HTMLElement): HTMLElement | null {
    ...
    if (tagName === "plank-select-item") {
      return this._renderItem(child as PlankSelectItem)
    } else if (tagName === "plank-select-group") {
      return this._renderGroup(child as PlankSelectGroup)
    } else if (tagName === "plank-select-label") {
      return this._renderLabel(child as PlankSelectLabel)
    } else if (tagName === "plank-select-separator") {
  ...
  private _renderItem(item: PlankSelectItem): HTMLElement {
  ...
  private _renderGroup(group: PlankSelectGroup): HTMLElement {
    const el = document.createElement("div")
  ...
  private _renderLabel(label: PlankSelectLabel): HTMLElement {
    const el = document.createElement("div")
...
/**
 * PlankSelectItem - A select item
 */
@customElement("plank-select-item")
export class PlankSelectItem extends LitElement {
  @property({ type: String, reflect: true }) value = ""
  @property({ type: Boolean, reflect: true }) disabled = false
...
/**
 * PlankSelectGroup - A group of select items
 */
@customElement("plank-select-group")
export class PlankSelectGroup extends LitElement {
  createRenderRoot() {
    return this
  }
...
/**
 * PlankSelectLabel - A label for a group of select items
 */
@customElement("plank-select-label")
export class PlankSelectLabel extends LitElement {
  createRenderRoot() {
    return this
  }
...
/**
 * PlankSelectSeparator - A separator between select items
 */
@customElement("plank-select-separator")
export class PlankSelectSeparator extends LitElement {
  createRenderRoot() {
    return this
  }
...
declare global {
  interface HTMLElementTagNameMap {
    "plank-select": PlankSelect
    "plank-select-trigger": PlankSelectTrigger
    "plank-select-value": PlankSelectValue
    "plank-select-content": PlankSelectContent
    "plank-select-item": PlankSelectItem
    "plank-select-group": PlankSelectGroup
    "plank-select-label": PlankSelectLabel
    "plank-select-separator": PlankSelectSeparator
  }
}
