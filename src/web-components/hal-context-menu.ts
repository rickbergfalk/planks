...
/**
 * HalContextMenu - Root container that manages context menu state
 *
 ...
 * ```
 */
@customElement("hal-context-menu")
export class HalContextMenu extends LitElement {
  @property({ type: Boolean, reflect: true }) open = false

  private _trigger: HalContextMenuTrigger | null = null
  private _content: HalContextMenuContent | null = null
  private _contentId = `hal-context-menu-content-${++contextMenuId}`
  ...
  private _updateChildren() {
    this._trigger = this.querySelector(
      "hal-context-menu-trigger"
    ) as HalContextMenuTrigger | null
    this._content = this.querySelector(
      "hal-context-menu-content"
    ) as HalContextMenuContent | null
  ...
  _dispatchSelect(item: HalContextMenuItem) {
    this.dispatchEvent(
  ...
  _dispatchCheckedChange(item: HalContextMenuCheckboxItem, checked: boolean) {
    this.dispatchEvent(
...
/**
 * HalContextMenuTrigger - Element that triggers the context menu on right-click
 */
@customElement("hal-context-menu-trigger")
export class HalContextMenuTrigger extends LitElement {
  private _contextMenu: HalContextMenu | null = null
  ...
  _setContextMenu(contextMenu: HalContextMenu) {
...
/**
 * HalContextMenuContent - The floating context menu content
 */
@customElement("hal-context-menu-content")
export class HalContextMenuContent extends LitElement {
  ...
  private _renderChild(child: HTMLElement): HTMLElement | null {
    ...
    if (tagName === "hal-context-menu-item") {
      return this._renderMenuItem(child as HalContextMenuItem)
    } else if (tagName === "hal-context-menu-checkbox-item") {
      return this._renderCheckboxItem(child as HalContextMenuCheckboxItem)
    } else if (tagName === "hal-context-menu-radio-group") {
      return this._renderRadioGroup(child as HalContextMenuRadioGroup)
    } else if (tagName === "hal-context-menu-label") {
      return this._renderLabel(child as HalContextMenuLabel)
    } else if (tagName === "hal-context-menu-separator") {
      return this._renderSeparator()
    } else if (tagName === "hal-context-menu-group") {
      return this._renderGroup(child as HalContextMenuGroup)
    } else if (tagName === "hal-context-menu-sub") {
      return this._renderSub(child as HalContextMenuSub)
    }

    return null
  }

  private _renderMenuItem(item: HalContextMenuItem): HTMLElement {
  ...
  private _renderItemContent(el: HTMLElement, item: HTMLElement) {
    Array.from(item.childNodes).forEach((node) => {
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        if (element.tagName.toLowerCase() === "hal-context-menu-shortcut") {
  ...
  private _renderCheckboxItem(item: HalContextMenuCheckboxItem): HTMLElement {
    const el = document.createElement("div")
  ...
  private _renderRadioGroup(group: HalContextMenuRadioGroup): HTMLElement {
    const el = document.createElement("div")
    el.setAttribute("role", "group")
    el.dataset.slot = "context-menu-radio-group"

    const radioItems = group.querySelectorAll("hal-context-menu-radio-item")
    radioItems.forEach((item) => {
      const radioEl = this._renderRadioItem(
        item as HalContextMenuRadioItem,
        group.value
      )
  ...
  private _renderRadioItem(
    ...
    if (!item.disabled) {
      el.addEventListener("click", () => {
        ...
        const radioGroup = item.closest("hal-context-menu-radio-group") as
          | HalContextMenuRadioGroup
          | undefined
        if (radioGroup) {
  ...
  private _renderLabel(item: HalContextMenuLabel): HTMLElement {
    const el = document.createElement("div")
  ...
  private _renderGroup(group: HalContextMenuGroup): HTMLElement {
    const el = document.createElement("div")
    el.setAttribute("role", "group")
  ...
  private _renderSub(sub: HalContextMenuSub): HTMLElement {
    const el = document.createElement("div")
    el.dataset.slot = "context-menu-sub"

    const subTrigger = sub.querySelector("hal-context-menu-sub-trigger")
    const subContent = sub.querySelector("hal-context-menu-sub-content")

    if (subTrigger) {
      const triggerEl = this._renderSubTrigger(
        subTrigger as HalContextMenuSubTrigger
      )
      el.appendChild(triggerEl)

      if (subContent && sub.open) {
        const contentEl = this._renderSubContent(
          subContent as HalContextMenuSubContent,
          triggerEl
        )
  ...
  private _renderSubTrigger(trigger: HalContextMenuSubTrigger): HTMLElement {
    const el = document.createElement("div")
    el.setAttribute("role", "menuitem")
    el.setAttribute("tabindex", "-1")
  ...
  private _renderSubContent(
    content: HalContextMenuSubContent,
...
/**
 * HalContextMenuItem - A menu item
 */
@customElement("hal-context-menu-item")
export class HalContextMenuItem extends LitElement {
...
/**
 * HalContextMenuCheckboxItem - A checkbox menu item
 */
@customElement("hal-context-menu-checkbox-item")
export class HalContextMenuCheckboxItem extends LitElement {
...
/**
 * HalContextMenuRadioGroup - A radio group container
 */
@customElement("hal-context-menu-radio-group")
export class HalContextMenuRadioGroup extends LitElement {
...
/**
 * HalContextMenuRadioItem - A radio menu item
 */
@customElement("hal-context-menu-radio-item")
export class HalContextMenuRadioItem extends LitElement {
  @property({ type: String, reflect: true }) value = ""
  @property({ type: Boolean, reflect: true }) disabled = false
...
/**
 * HalContextMenuLabel - A label for menu items
 */
@customElement("hal-context-menu-label")
export class HalContextMenuLabel extends LitElement {
  @property({ type: Boolean, reflect: true }) inset = false
...
/**
 * HalContextMenuSeparator - A separator between menu items
 */
@customElement("hal-context-menu-separator")
export class HalContextMenuSeparator extends LitElement {
  createRenderRoot() {
    return this
  }
...
/**
 * HalContextMenuShortcut - A keyboard shortcut hint
 */
@customElement("hal-context-menu-shortcut")
export class HalContextMenuShortcut extends LitElement {
  createRenderRoot() {
    return this
  }
...
/**
 * HalContextMenuGroup - A group of menu items
 */
@customElement("hal-context-menu-group")
export class HalContextMenuGroup extends LitElement {
  createRenderRoot() {
    return this
  }
...
/**
 * HalContextMenuSub - A submenu container
 */
@customElement("hal-context-menu-sub")
export class HalContextMenuSub extends LitElement {
  @property({ type: Boolean, reflect: true }) open = false
...
/**
 * HalContextMenuSubTrigger - A trigger for submenu
 */
@customElement("hal-context-menu-sub-trigger")
export class HalContextMenuSubTrigger extends LitElement {
  @property({ type: Boolean, reflect: true }) inset = false
...
/**
 * HalContextMenuSubContent - The content of a submenu
 */
@customElement("hal-context-menu-sub-content")
export class HalContextMenuSubContent extends LitElement {
  createRenderRoot() {
    return this
  }
...
declare global {
  interface HTMLElementTagNameMap {
    "hal-context-menu": HalContextMenu
    "hal-context-menu-trigger": HalContextMenuTrigger
    "hal-context-menu-content": HalContextMenuContent
    "hal-context-menu-item": HalContextMenuItem
    "hal-context-menu-checkbox-item": HalContextMenuCheckboxItem
    "hal-context-menu-radio-group": HalContextMenuRadioGroup
    "hal-context-menu-radio-item": HalContextMenuRadioItem
    "hal-context-menu-label": HalContextMenuLabel
    "hal-context-menu-separator": HalContextMenuSeparator
    "hal-context-menu-shortcut": HalContextMenuShortcut
    "hal-context-menu-group": HalContextMenuGroup
    "hal-context-menu-sub": HalContextMenuSub
    "hal-context-menu-sub-trigger": HalContextMenuSubTrigger
    "hal-context-menu-sub-content": HalContextMenuSubContent
  }
}
