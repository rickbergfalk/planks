...
/**
 * PlankResizablePanelGroup - Container for resizable panels
 *
 * @example
 * ```html
 * <plank-resizable-panel-group direction="horizontal" class="rounded-lg border">
 ...
 * ```
 */
@customElement("plank-resizable-panel-group")
export class PlankResizablePanelGroup extends LitElement {
  @property({ type: String }) direction: "horizontal" | "vertical" =
    "horizontal"
  @property({ type: String }) class: string = ""

  // Not using @state since these don't need to trigger re-renders
  private _panels: PlankResizablePanel[] = []
  private _handles: PlankResizableHandle[] = []
  ...
  private _collectPanels() {
    this._panels = Array.from(
      this.querySelectorAll(":scope > plank-resizable-panel")
    ) as PlankResizablePanel[]
    this._handles = Array.from(
      this.querySelectorAll(":scope > plank-resizable-handle")
  ...
  private _initializePanelSizes() {
    ...
    let totalSize = 0
    const panelsWithoutDefault: PlankResizablePanel[] = []
...
/**
 * PlankResizablePanel - A resizable panel within a group
 */
@customElement("plank-resizable-panel")
export class PlankResizablePanel extends LitElement {
  ...
  private _group: PlankResizablePanelGroup | null = null

  createRenderRoot() {
    return this
  }

  _setGroup(group: PlankResizablePanelGroup) {
    this._group = group
  }
...
/**
 * PlankResizableHandle - A drag handle for resizing panels
 */
@customElement("plank-resizable-handle")
export class PlankResizableHandle extends LitElement {
  ...
  _setGroup(group: PlankResizablePanelGroup, index: number) {
    this._group = group
...
declare global {
  interface HTMLElementTagNameMap {
    "plank-resizable-panel-group": PlankResizablePanelGroup
    "plank-resizable-panel": PlankResizablePanel
    "plank-resizable-handle": PlankResizableHandle
  }
}
