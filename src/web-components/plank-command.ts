...
/**
 * PlankCommand - Root container for command palette
 *
 ...
 * ```
 */
@customElement("plank-command")
export class PlankCommand extends LitElement {
  @state() private _searchValue = ""
  @state() private _selectedIndex = 0

  private _instanceId = `plank-command-${++commandId}`
  private _input: PlankCommandInput | null = null
  private _list: PlankCommandList | null = null
  private _items: PlankCommandItem[] = []
  private _visibleItems: PlankCommandItem[] = []
  ...
  private _updateChildren() {
    this._input = this.querySelector(
      "plank-command-input"
    ) as PlankCommandInput | null
    this._list = this.querySelector(
      "plank-command-list"
    ) as PlankCommandList | null
  ...
  private _collectItems() {
    this._items = Array.from(
      this.querySelectorAll("plank-command-item")
    ) as PlankCommandItem[]
  ...
  private _filterItems() {
    ...
    const empty = this.querySelector(
      "plank-command-empty"
    ) as PlankCommandEmpty | null
    if (empty) {
      empty._setVisible(this._visibleItems.length === 0)
    }

    // Update groups
    const groups = this.querySelectorAll("plank-command-group")
    groups.forEach((group) => {
      const groupEl = group as PlankCommandGroup
      const items = groupEl.querySelectorAll("plank-command-item")
      const hasVisibleItems = Array.from(items).some((item) =>
        (item as PlankCommandItem)._isVisible()
      )
  ...
  _selectItem(item: PlankCommandItem) {
...
/**
 * PlankCommandInput - Search input for command palette
 */
@customElement("plank-command-input")
export class PlankCommandInput extends LitElement {
  @property({ type: String }) placeholder = ""
  @property({ type: String }) class: string = ""

  private _command: PlankCommand | null = null
  ...
  _setCommand(command: PlankCommand) {
...
/**
 * PlankCommandList - Container for command items
 */
@customElement("plank-command-list")
export class PlankCommandList extends LitElement {
  @property({ type: String }) class: string = ""
...
/**
 * PlankCommandEmpty - Shown when no items match search
 */
@customElement("plank-command-empty")
export class PlankCommandEmpty extends LitElement {
  @property({ type: String }) class: string = ""

  // Start hidden - PlankCommand will show us if needed
...
/**
 * PlankCommandGroup - Groups items with a heading
 */
@customElement("plank-command-group")
export class PlankCommandGroup extends LitElement {
...
/**
 * PlankCommandItem - A command item
 */
@customElement("plank-command-item")
export class PlankCommandItem extends LitElement {
...
/**
 * PlankCommandSeparator - Separator between items
 */
@customElement("plank-command-separator")
export class PlankCommandSeparator extends LitElement {
  @property({ type: String }) class: string = ""
...
/**
 * PlankCommandShortcut - Keyboard shortcut display
 */
@customElement("plank-command-shortcut")
export class PlankCommandShortcut extends LitElement {
  @property({ type: String }) class: string = ""
...
declare global {
  interface HTMLElementTagNameMap {
    "plank-command": PlankCommand
    "plank-command-input": PlankCommandInput
    "plank-command-list": PlankCommandList
    "plank-command-empty": PlankCommandEmpty
    "plank-command-group": PlankCommandGroup
    "plank-command-item": PlankCommandItem
    "plank-command-separator": PlankCommandSeparator
    "plank-command-shortcut": PlankCommandShortcut
  }
}
