...
/**
 * HalTooltip - Root container that manages tooltip state
 *
 ...
 * ```
 */
@customElement("hal-tooltip")
export class HalTooltip extends LitElement {
  @property({ type: Boolean, reflect: true }) open = false
  @property({ type: Number }) delayDuration = 0

  private _trigger: HalTooltipTrigger | null = null
  private _content: HalTooltipContent | null = null
  private _contentId = `hal-tooltip-content-${++tooltipId}`
  ...
  private _updateChildren() {
    this._trigger = this.querySelector(
      "hal-tooltip-trigger"
    ) as HalTooltipTrigger | null
    this._content = this.querySelector(
      "hal-tooltip-content"
    ) as HalTooltipContent | null
...
/**
 * HalTooltipTrigger - Element that triggers the tooltip on hover/focus
 */
@customElement("hal-tooltip-trigger")
export class HalTooltipTrigger extends LitElement {
  private _tooltip: HalTooltip | null = null
  ...
  _setTooltip(tooltip: HalTooltip) {
...
/**
 * HalTooltipContent - The floating tooltip content
 */
@customElement("hal-tooltip-content")
export class HalTooltipContent extends LitElement {
...
declare global {
  interface HTMLElementTagNameMap {
    "hal-tooltip": HalTooltip
    "hal-tooltip-trigger": HalTooltipTrigger
    "hal-tooltip-content": HalTooltipContent
  }
}
