...
/**
 * HalRadioGroup - Container component that manages radio selection state
 *
 ...
 * ```
 */
@customElement("hal-radio-group")
export class HalRadioGroup extends LitElement {
  ...
  private _updateChildren() {
    const items = this.querySelectorAll("hal-radio-group-item")
    const itemsArray = Array.from(items) as HalRadioGroupItem[]
  ...
  _getItems(): HalRadioGroupItem[] {
    return Array.from(
      this.querySelectorAll("hal-radio-group-item")
    ) as HalRadioGroupItem[]
  }

  _navigateItems(direction: 1 | -1, currentItem: HalRadioGroupItem) {
...
/**
 * HalRadioGroupItem - Individual radio button
 */
@customElement("hal-radio-group-item")
export class HalRadioGroupItem extends LitElement {
  @property({ type: String, reflect: true }) value = ""
  @property({ type: Boolean, reflect: true }) disabled = false
  ...
  private _handleClick = () => {
    if (this.disabled) return
    const group = this.closest("hal-radio-group") as HalRadioGroup
...
declare global {
  interface HTMLElementTagNameMap {
    "hal-radio-group": HalRadioGroup
    "hal-radio-group-item": HalRadioGroupItem
  }
}
