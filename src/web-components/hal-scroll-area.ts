...
/**
 * HalScrollArea - A scrollable area with custom styled scrollbars
 *
 * Can be used simply (like React) or with explicit structure:
 *
 * Simple (auto-wraps content, adds vertical scrollbar):
 * ```html
 * <hal-scroll-area class="h-48 w-48 rounded-md border">
 ...
 * ```
 */
@customElement("hal-scroll-area")
export class HalScrollArea extends LitElement {
  ...
  private _injectGlobalStyles() {
    const styleId = "hal-scroll-area-styles"
    if (!document.getElementById(styleId)) {
      const style = document.createElement("style")
      style.id = styleId
      style.textContent = `
        [data-hal-scroll-viewport] {
          scrollbar-width: none;
          -ms-overflow-style: none;
          -webkit-overflow-scrolling: touch;
        }
        [data-hal-scroll-viewport]::-webkit-scrollbar {
          display: none;
        }
      `
  ...
  private _autoWrapContent() {
    ...
    const existingViewport = this.querySelector("hal-scroll-area-viewport")
    ...
    const children = Array.from(this.childNodes).filter((node) => {
      ...
      if (node instanceof Element) {
        ...
        if (
          tagName === "hal-scroll-bar" ||
          tagName === "hal-scroll-area-corner"
        )
          return false
      }
      return true
    ...
    const viewport = document.createElement("hal-scroll-area-viewport")
    viewport.setAttribute("data-hal-scroll-viewport", "")
    ...
    if (!this.querySelector("hal-scroll-bar")) {
      const scrollbar = document.createElement("hal-scroll-bar")
  ...
  firstUpdated() {
    this._viewport = this.querySelector("[data-hal-scroll-viewport]")
    this._content = this._viewport?.querySelector(
      "[data-hal-scroll-content]"
  ...
  private _notifyScrollbars() {
    const scrollbars = this.querySelectorAll("hal-scroll-bar")
    scrollbars.forEach((sb) => {
      const scrollbar = sb as HalScrollBar
...
/**
 * HalScrollAreaViewport - The scrollable viewport container
 */
@customElement("hal-scroll-area-viewport")
export class HalScrollAreaViewport extends LitElement {
  @property({ type: String }) class: string = ""

  private _scrollArea: HalScrollArea | null = null
  ...
  connectedCallback() {
    super.connectedCallback()
    this._scrollArea = this.closest("hal-scroll-area") as HalScrollArea

    // Set viewport attribute immediately so parent can find us
    this.setAttribute("data-hal-scroll-viewport", "")

    // Wrap children in content div synchronously to ensure proper layout
    if (!this.querySelector("[data-hal-scroll-content]")) {
      const contentWrapper = document.createElement("div")
      contentWrapper.setAttribute("data-hal-scroll-content", "")
  ...
  willUpdate() {
    ...
    const hasVerticalScrollbar = this._scrollArea?.querySelector(
      'hal-scroll-bar:not([orientation="horizontal"])'
    )
...
/**
 * HalScrollBar - Custom scrollbar component
 */
@customElement("hal-scroll-bar")
export class HalScrollBar extends LitElement {
  ...
  private _setupResizeObserver() {
    ...
    if (this._viewport) {
      ...
      const content = this._viewport.querySelector("[data-hal-scroll-content]")
      if (content) {
...
/**
 * HalScrollAreaCorner - Corner element when both scrollbars are visible
 */
@customElement("hal-scroll-area-corner")
export class HalScrollAreaCorner extends LitElement {
  @property({ type: String }) class: string = ""
...
declare global {
  interface HTMLElementTagNameMap {
    "hal-scroll-area": HalScrollArea
    "hal-scroll-area-viewport": HalScrollAreaViewport
    "hal-scroll-bar": HalScrollBar
    "hal-scroll-area-corner": HalScrollAreaCorner
  }
}
